...
class NotifyHelper {
  ...
  scheduledNotification(int hour, int minutes, Task task) async {
    await flutterLocalNotificationsPlugin.zonedSchedule(
      ...
      _nextInstanceOfTenAM(
          hour, minutes, task.remind!, task.repeat!, task.dates!),
  ...
  tz.TZDateTime _nextInstanceOfTenAM(
    ...
    DateTime currentDate = DateFormat.yMd().parse(date);
    tz.TZDateTime scheduledDate =
        tz.TZDateTime(tz.local, now.year, now.month, now.day, hour, minutes);

    final tz.TZDateTime fd = tz.TZDateTime.from(scheduledDate, tz.local);

    scheduledDate = myRemind(remind, scheduledDate);
    ...
    print('New Date: $scheduledDate');
    return scheduledDate;
  }

  tz.TZDateTime myRemind(int remind, tz.TZDateTime scheduledDate) {
    if (remind == 5) {
